...
// Informationen ändern
	if ($_POST['action'] == 'infoaendern') {
		...
		if ($_POST['lstScanTyp'] != '') {
			$SQL_Result = tic_mysql_query('UPDATE `gn4accounts` SET scananfragen="'.$_POST['scananfragen'].'", svs="'.$_POST['txtSVs'].'", sbs="'.$_POST['txtSBs'].'", scantyp="'.$_POST['lstScanTyp'].'" WHERE id="'.$Benutzer['id'].'";', $SQL_DBConn) or die(mysql_errno()." - ".mysql_error());
